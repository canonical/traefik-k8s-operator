...
- Your charm either uses open-port<sup>[1](https://juju.is/docs/sdk/hook-tool#open-port),[2](https://github.com/canonical/operator/pull/905)</sup> or applies a [Kubernetes service patch](https://github.com/canonical/observability-libs/blob/main/lib/charms/observability_libs/v1/kubernetes_service_patch.py) for exposing the workload's port. 
...
This means that you need to tell your workload about it (for example alertmanager takes a `--web.external-url` cli arg)
