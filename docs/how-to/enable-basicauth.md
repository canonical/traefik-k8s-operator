...
```{note}
We don't yet support multiple users or per-route auth. Need that? [Let us know!](https://github.com/canonical/traefik-k8s-operator/issues) For the time being, you can consider deploying multiple `traefik`s to segment your namespace with separate user domains.
```
