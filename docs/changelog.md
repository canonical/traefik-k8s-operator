...
- Fixed several documentation issues:
  ...
  - Fixed inconsistent/missing docstrings in the `ingress` (v2) and `ingress_per_unit` (v1) charm
    ...
    constructor argument (fixes [#640](https://github.com/canonical/traefik-k8s-operator/issues/640)).
  ...
  - Corrected the `traefik-route` relation description in `metadata.yaml`, which incorrectly
    described the relation as being configured "on a per-unit basis" (fixes [#511](https://github.com/canonical/traefik-k8s-operator/issues/511)).
  ...
  - Documented that deploying Traefik without `--trust` causes the unit to enter `error` status
    (an uncaught Kubernetes `Forbidden` API error while reconciling the LoadBalancer service),
    and explained how to recover, in the tutorial and a new troubleshooting guide
    (fixes [#640](https://github.com/canonical/traefik-k8s-operator/issues/640)).
...
- Added a `base` input to the Terraform module to set the OS base the charm is deployed on.
