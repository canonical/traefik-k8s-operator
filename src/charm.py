...
class TraefikIngressCharm(CharmBase):
    ...
    def _on_certificate_expiring(self, event: CertificateExpiringEvent) -> None:
        ...
        new_csr = generate_csr(
            private_key=self.secret_store.private_key.get_content()["private-key"].encode(),
            subject=subject,
        )
...
class SecretStore:
    ...
    def store_private_key(self, private_key: str) -> None:
        """Stores the private key in the appropriate backend.
    ...
    def using_juju_secrets(self) -> bool:
        """Checks if the used Juju version has the secrets feature.

        Returns:
            bool: True if Juju secrets are available.
        """
    ...
    def migrate_private_key(self) -> Optional[str]:
        ...
        logger.debug("Deleting private key from stored state.")
        self.charm._stored.private_key = None
        logger.debug("Private key migrated successfully.")
