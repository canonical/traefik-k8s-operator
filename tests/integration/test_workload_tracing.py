...
def _deploy_tempo_cluster(juju: jubilant.Juju) -> None:
    ...
    # against it below.
    juju.wait(jubilant.all_agents_idle, error=any_error_after(failures=5), delay=5, successes=5)
    ...
    juju.wait(all_settled, error=any_error_after(failures=5), timeout=2000, delay=5, successes=5)
