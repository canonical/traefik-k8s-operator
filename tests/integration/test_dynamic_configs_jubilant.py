...
def deploy_catalogue(juju: jubilant.Juju) -> None:
...
def _list_dynamic_configs(juju: jubilant.Juju) -> list[str]:
...
def test_deploy(juju: jubilant.Juju, charm: Path):
    """Deploy traefik, alertmanager, and catalogue."""
    deploy_traefik(juju, charm)
    deploy_alertmanager(juju)
    deploy_catalogue(juju)
...
def test_dynamic_configs_present(juju: jubilant.Juju):
...
def test_dynamic_config_content_valid(juju: jubilant.Juju):
...
def test_staging_artifacts_cleaned_up(juju: jubilant.Juju):
...
def test_dynamic_config_removed_after_relation_removed(juju: jubilant.Juju):
