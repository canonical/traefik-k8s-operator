...
from charms.traefik_k8s.v1.ingress_per_unit import (
    IngressPerUnitRequirer, IngressPerUnitReadyEvent,
    IngressPerUnitReadyForUnitEvent,
)
from tests.unit.capture import capture_events, capture
...
def _requirer_provide_ingress(
        harness: Harness[MockRequirerCharm],
        unit_name: str, url: str, relation: Relation):
...
def test_ingress_unit_requirer_related(requirer, harness, url, leader):
    ...
    with capture_events(harness.charm) as captured:
        _requirer_provide_ingress(harness, harness.charm.unit.name, url,
                                  relation)
...
def test_unit_joining_does_not_trigger_ingress_changed(requirer, harness, url):
    ...
    with capture(harness.charm, IngressPerUnitReadyForUnitEvent):
        _requirer_provide_ingress(harness, harness.charm.unit.name,
                                  'a_different_url.com',
                                  relation)
